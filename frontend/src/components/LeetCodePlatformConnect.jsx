...
const LeetCodePlatformConnect = ({ onConnect, onError }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // First verify the username exists on LeetCode
      ...
      if (response.data && response.data.success) {
        try {
          await platformsAPI.syncPlatform('leetcode');
        } catch (syncErr) {
      ...
      setIsLoading(false);
      const errorMessage = err.response?.data?.msg || 'Failed to connect to LeetCode';
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h2 className="text-xl font-semibold mb-4">Connect to LeetCode</h2>
...
export default LeetCodePlatformConnect;